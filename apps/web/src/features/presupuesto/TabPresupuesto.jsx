...
import { CATEGORIAS, SUBCAT_VAR, COLOR_VIAJE, BG_VIAJE } from "../../constants/categorias.js";
import { C, cardN } from "../../constants/colores.js";
import { MESES, MESES_CORTO } from "../../constants/meses.js";
import { fmt, fmtd, labelMes } from "../../utils/format.js";
import { todayISO, addMeses, daysBetween } from "../../utils/dates.js";
import { calcDeuda } from "../../utils/calcDeuda.js";
import { useDatosMes, calcCuotaDeudaMes } from "../../hooks/useDatosMes.js";
import { useBreakpoint } from "../../hooks/useBreakpoint.js";
import { BASE } from "../../data/demo.js";
import PanelDeudas from "./PanelDeudas.jsx";
import SeccionGastosVariables from "./SeccionGastosVariables.jsx";
import ModalPalanca from "./modals/ModalPalanca.jsx";
import ModalDeuda from "./modals/ModalDeuda.jsx";
    ...
    <div style={{ display:"grid", gap:20, minWidth:0 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:kpiColumns, gap:isMobile?10:12 }}>
        <div style={{ ...cardN(), borderTop:`3px solid ${C.cyan}` }}>
          <div style={{ fontSize:10,fontWeight:700,color:C.cyan,textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:6 }}>💶 Ingresos fijos</div>
          <div style={{ fontSize:26,fontWeight:700,color:C.txt,fontFamily:"'Playfair Display',serif" }}>{fmt(BASE.ingresos_fijos)}</div>
          <div style={{ fontSize:11,color:C.txt2,marginTop:3 }}>mensual garantizado</div>
        </div>
        <div style={{ ...cardN(), borderTop:`3px solid ${C.lavender}` }}>
          <div style={{ fontSize:10,fontWeight:700,color:C.lavender,textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:6 }}>📈 Ingresos variables</div>
          <div style={{ fontSize:26,fontWeight:700,color:C.txt,fontFamily:"'Playfair Display',serif" }}>{fmt(totales.varAnual)}</div>
          <div style={{ fontSize:11,color:C.txt2,marginTop:3 }}>acumulado {año}</div>
        </div>
        <div style={{ ...cardN(), borderTop:`3px solid ${C.sage}`, background:`linear-gradient(135deg,${C.superficie},${C.sageLight})` }}>
          <div style={{ fontSize:10,fontWeight:700,color:C.sageDark,textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:6 }}>✨ Potencial sin activar</div>
          <div style={{ fontSize:26,fontWeight:700,color:C.sageDark,fontFamily:"'Playfair Display',serif" }}>{fmt(totales.potencial)}</div>
          <div style={{ fontSize:11,color:C.sageDark,opacity:0.7,marginTop:3 }}>{palancas.filter(p=>!p.activa).length} palanca{palancas.filter(p=>!p.activa).length!==1?"s":""} inactiva{palancas.filter(p=>!p.activa).length!==1?"s":""}</div>
        </div>
        <div style={{ ...cardN(), borderTop:`3px solid ${C.warn}` }}>
          <div style={{ fontSize:10,fontWeight:700,color:"#b45309",textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:6 }}>📌 Gastos fijos</div>
          ...
          <div style={{ fontSize:11,color:C.txt2,marginTop:3 }}>estructura mensual</div>
        </div>
        <div style={{ ...cardN(), borderTop:`3px solid ${totales.presionActual>85?C.error:totales.presionActual>70?C.warn:C.exito}` }}>
          <div style={{ fontSize:10,fontWeight:700,color:C.txt2,textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:6 }}>⚡ Presión financiera</div>
          ...
          <div style={{ fontSize:11,color:C.txt2,marginTop:3 }}>del ingreso comprometido</div>
        ...
        <div style={{ ...cardN(), background:"linear-gradient(135deg,#1e1a2e,#2d1f3d)", border:"none", cursor:"pointer" }} onClick={()=>setShowDeudas(true)}>
          <div style={{ fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.45)",textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:6 }}>💳 Deuda pendiente</div>
          <div style={{ fontSize:26,fontWeight:700,color:C.warn,fontFamily:"'Playfair Display',serif" }}>{fmt(totalPendiente)}</div>
          <div style={{ fontSize:11,color:"rgba(255,255,255,0.35)",marginTop:3 }}>{fmt(cuotaMesActual)}/mes · {deudas.length} deuda{deudas.length!==1?"s":""}</div>
          {proxVencimiento && (
            <div style={{ marginTop:8,fontSize:10,color:"rgba(255,255,255,0.3)" }}>
              Próx. fin: {proxVencimiento.nombre} · {labelMes(addMeses(proxVencimiento.mes_inicio, proxVencimiento.cuotas_totales-1))}
            </div>
          )}
        </div>
      </div>
      ...
      <div style={cardN(isMobile ? { padding:"14px 12px" } : undefined)}>
        <div style={{ fontSize:16,fontWeight:700,color:C.txt,marginBottom:4 }}>Estructura de ingresos</div>
        <div style={{ fontSize:12,color:C.txt2,marginBottom:16 }}>Fijos garantizados · Variables registrados · Potencial activable</div>
        <div style={{ display:"grid", gridTemplateColumns:threeColumns, gap:isMobile?12:16 }}>
          ...
          <div>
            <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:12,padding:"8px 12px",background:C.cyanLight,borderRadius:10,border:`1px solid ${C.cyan}33` }}>
              ...
              <span style={{ fontSize:12,fontWeight:700,color:C.cyan,textTransform:"uppercase",letterSpacing:"0.6px" }}>Ingresos fijos</span>
            </div>
            <div style={{ display:"grid", gap:5 }}>
              ...
              <div style={{ display:"flex",justifyContent:"space-between",fontSize:13,padding:"9px 12px",background:C.cyan,borderRadius:9,color:"white",marginTop:2 }}>
                <span style={{ fontWeight:700 }}>Total mensual</span><span style={{ fontWeight:700 }}>{fmt(BASE.ingresos_fijos)}</span>
              </div>
            </div>
          </div>
          ...
          <div>
            <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:12,padding:"8px 12px",background:C.lavLight,borderRadius:10,border:`1px solid ${C.lavender}33` }}>
              ...
              <span style={{ fontSize:12,fontWeight:700,color:C.lavender,textTransform:"uppercase",letterSpacing:"0.6px" }}>Variables ({MESES[mesActual]})</span>
            </div>
            <div style={{ display:"grid", gap:5 }}>
              {Object.entries(SUBCAT_VAR).map(([k,v]) => {
                ...
                return (
                  <div key={k} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:13,padding:"8px 12px",background:val>0?v.bg:C.fondo,borderRadius:9,border:`1px solid ${val>0?v.color+"33":C.borde}` }}>
                    <span style={{ color:val>0?v.color:C.txt2 }}>{v.emoji} {v.label}</span>
              ...
              <div style={{ display:"flex",justifyContent:"space-between",fontSize:13,padding:"9px 12px",background:C.lavender,borderRadius:9,color:"white",marginTop:2 }}>
                <span style={{ fontWeight:700 }}>Total este mes</span><span style={{ fontWeight:700 }}>{fmt(datosMes[mesActual]?.ingresos_var_total||0)}</span>
              </div>
            </div>
          </div>
          ...
          <div>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,padding:"8px 12px",background:C.sageLight,borderRadius:10,border:`1px solid ${C.sage}44` }}>
              <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                ...
                <span style={{ fontSize:12,fontWeight:700,color:C.sageDark,textTransform:"uppercase",letterSpacing:"0.6px" }}>Palancas</span>
              </div>
              <button onClick={()=>setModalPalanca({})} style={{ background:C.sage,color:"white",border:"none",borderRadius:8,padding:"3px 10px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'Lato',sans-serif" }}>+ Nueva</button>
            </div>
            <div style={{ display:"grid", gap:6 }}>
              {palancas.length === 0 && <div style={{ textAlign:"center",padding:"20px 0",fontSize:12,color:C.txt2 }}>Sin palancas aún</div>}
              {palancas.map(p => {
                ...
                return (
                  <div key={p.id} style={{ padding:"9px 12px",background:p.activa?C.sageLight:C.fondo,borderRadius:10,border:`1px solid ${p.activa?C.sage+"66":C.borde}`,transition:"all 0.2s" }}>
                    ...
                    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                      <span style={{ fontSize:10,padding:"2px 8px",borderRadius:10,background:sub?.bg,color:sub?.color,fontWeight:600 }}>{sub?.label}</span>
      ...
      <div style={cardN(isMobile ? { padding:"14px 12px" } : undefined)}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4,flexDirection:isMobile?"column":"row",gap:isMobile?8:12 }}>
          <div>
            <div style={{ fontSize:16,fontWeight:700,color:C.txt }}>Gastos por capa — {año}</div>
            <div style={{ fontSize:12,color:C.txt2,marginTop:2 }}>Pasá el mouse sobre cada barra · futuro en gris estimado</div>
          </div>
          <div style={{ display:"flex",gap:12,alignItems:"center",flexWrap:"wrap" }}>
            {[{color:"#64748b",label:"① Estructural"},{color:"#d97706",label:"② Suministros"},{color:C.lavender,label:"③ Discrecional"}].map(l=>(
              ...
              <div key={i}
                ...
                {showTip && (
                  <div style={{ position:"absolute",bottom:"100%",left:"50%",transform:"translateX(-50%)",zIndex:100,background:"#1a1a2e",borderRadius:10,padding:"10px 12px",minWidth:160,marginBottom:6,pointerEvents:"none",boxShadow:"0 4px 16px rgba(0,0,0,0.3)" }}>
                    <div style={{ fontSize:11,fontWeight:700,color:"white",marginBottom:6 }}>{MESES[i]} {año}{m.esFuturo?" · estimado":""}</div>
                    {[
                      {l:"① Estructural",  v:fmt(m.gasto_estructural),  c:"#94a3b8"},
                      {l:"② Suministros",  v:fmt(m.gasto_suministros),  c:"#fbbf24"},
                      {l:"③ Discrecional", v:fmt(m.gasto_discrecional), c:C.lavender},
                      {l:"Ingresos totales",v:fmt(m.total_ingresos),    c:C.exito},
                    ].map(x=>(
                    ...
                    <div style={{ display:"flex",justifyContent:"space-between",gap:12 }}>
                      <span style={{ fontSize:10,color:"rgba(255,255,255,0.5)" }}>Saldo</span>
                ...
                <div style={{ fontSize:9,color:m.esActual?C.cyan:C.txt2,fontWeight:m.esActual?700:400 }}>{MESES_CORTO[i]}</div>
              </div>
            );
          })}
        </div>
        ...
        {detalle && (
          <div style={{ marginTop:16,padding:16,background:C.fondo,borderRadius:12,border:`1px solid ${C.borde}` }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14 }}>
              <span style={{ fontWeight:700,fontSize:15,color:C.txt }}>{MESES[detalle.mes]} {año}{detalle.esFuturo?" · estimado":""}</span>
            ...
            <div style={{ display:"grid",gridTemplateColumns:isMobile?"1fr":threeColumns,gap:8,marginBottom:12 }}>
              {[
                {l:"① Estructural",  v:fmt(detalle.gasto_estructural),  c:"#64748b", bg:"#f8fafc", sub:`Fijos ${fmt(BASE.gastos_fijos)} + Deudas ${fmt(detalle.gasto_deudas)}`},
                {l:"② Suministros",  v:fmt(detalle.gasto_suministros),  c:"#d97706", bg:"#fef3c7", sub:"Luz, gas, agua, internet..."},
                {l:"③ Discrecional", v:fmt(detalle.gasto_discrecional), c:C.lavender,bg:C.lavLight,sub:`Calendario ${fmt(detalle.gastos_var)} + Viajes ${fmt(detalle.gastos_viaje)}`},
              ].map(x=>(
            ...
            {detalle.palancasMes.length > 0 && (
              <div style={{ padding:"10px 12px",background:C.sageLight,borderRadius:10,border:`1px solid ${C.sage}44`,marginBottom:10 }}>
                <div style={{ fontSize:11,fontWeight:700,color:C.sageDark,marginBottom:6 }}>✨ Palancas activas</div>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:10 }}>
              <span style={{ fontSize:11,color:C.txt2,flexShrink:0 }}>Presión: {detalle.presion}%</span>
      ...
      {gastosCatAnual.length > 0 && (
        <div style={cardN(isMobile ? { padding:"14px 12px" } : undefined)}>
          <div style={{ fontSize:16,fontWeight:700,color:C.txt,marginBottom:14 }}>Gastos variables por categoría — {año}</div>
      ...
      {viajes.length > 0 && (
        <div style={cardN(isMobile ? { padding:"14px 12px" } : undefined)}>
          <div style={{ fontSize:16,fontWeight:700,color:C.txt,marginBottom:14 }}>✈️ Viajes {año}</div>
          <div style={{ display:"grid", gap:8 }}>
            {viajes.map(v=>{
              const total=Object.values(v.gastos||{}).reduce((a,b)=>a+b,0);
                ...
                <div key={v.id} style={{ padding:"12px 14px",background:C.fondo,borderRadius:12,border:`1px solid ${C.borde}` }}>
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
                    ...
                    <div style={{ textAlign:"right" }}>
                      ...
                      {v.presupuesto>0&&<div style={{ fontSize:11,color:C.txt2 }}>de {fmt(v.presupuesto)}</div>}
                    </div>
                  </div>
      ...
      {(() => {
        ...
        return(
          <div style={{ ...cardN(isMobile ? { padding:"14px 12px" } : undefined),background:"linear-gradient(135deg,#0f2420,#1a3d30)",border:"none" }}>
            <div style={{ fontSize:16,fontWeight:700,color:"white",marginBottom:4 }}>🔮 Proyección sin deudas</div>
            <div style={{ fontSize:12,color:"rgba(255,255,255,0.5)",marginBottom:16 }}>Liberación de cuotas mes a mes · impacto en saldo libre</div>
            <div style={{ display:"grid",gridTemplateColumns:threeColumns,gap:12,marginBottom:20 }}>
              {[
                {l:"Saldo libre hoy",       v:fmt(saldoHoy),      sub:"con todas las cuotas activas"},
                {l:"Saldo libre sin deudas",v:fmt(saldoFinal),    sub:"cuando estén todas saldadas"},
                {l:"Se liberan en total",   v:fmt(totalLiberado), sub:"€/mes cuando acaben"},
              ].map(x=>(
            ...
            <div style={{ borderRadius:12,overflowX:"auto",overflowY:"hidden",border:"1px solid rgba(255,255,255,0.1)" }}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:debtProjectionColumns,gap:0,background:"rgba(255,255,255,0.1)",padding:"8px 14px" }}>
                <div style={{ fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.5)",textTransform:"uppercase" }}>Mes</div>
                ...
                <div style={{ fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.5)",textTransform:"uppercase",textAlign:"right" }}>Cuotas</div>
                <div style={{ fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.5)",textTransform:"uppercase",textAlign:"right" }}>Saldo libre</div>
              </div>
                  ...
                  return(
                    <div key={f.pref}>
                      <div style={{ display:"grid",gridTemplateColumns:debtProjectionColumns,gap:0,padding:"9px 14px",background:hayLib?"rgba(117,223,144,0.12)":esHoy?"rgba(255,255,255,0.06)":fi%2===0?"rgba(255,255,255,0.02)":"transparent",borderTop:"1px solid rgba(255,255,255,0.05)",alignItems:"center" }}>
                        <div style={{ fontSize:12,fontWeight:esHoy?700:400,color:esHoy?C.cyan:"rgba(255,255,255,0.7)" }}>{esHoy?"→ ":""}{MESES_CORTO[+f.pref.split("-")[1]-1]} {f.pref.split("-")[0]}</div>
                        {f.detalleDeudas.map(d=>(
                          <div key={d.id} style={{ textAlign:"center" }}>
                            {d.esUltima?<span style={{ fontSize:11,fontWeight:700,color:C.exito }}>✓ última</span>
                              :d.activa?<div><div style={{ fontSize:11,fontWeight:600,color:"rgba(255,255,255,0.8)" }}>{fmt(d.cuotaMes)}</div><div style={{ fontSize:9,color:"rgba(255,255,255,0.35)",marginTop:1 }}>{d.pendientes-1} pend.</div></div>
                      ...
                      {hayLib&&<div style={{ padding:"6px 14px",background:"rgba(117,223,144,0.15)",borderTop:"1px solid rgba(117,223,144,0.2)" }}><span style={{ fontSize:11,color:C.exito,fontWeight:700 }}>🎉 {f.liberaciones.map(d=>d.nombre).join(" + ")} — última cuota · se liberan {fmt(f.liberaciones.reduce((a,d)=>a+d.cuota+(d.interes_mensual||0),0))}/mes</span></div>}
                    </div>
                  );
                })}
              </div>
