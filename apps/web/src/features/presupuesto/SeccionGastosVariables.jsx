...
import { CATEGORIAS, SUMINISTROS_TIPOS, COLOR_VIAJE, BG_VIAJE } from "../../constants/categorias.js";
...
export default function SeccionGastosVariables({ eventos, viajes, año, mesActual, suministros, setSuministros }) {
  ...
  const gastoViajeMes = viajesMes.reduce((a, v) => a + Object.values(v.gastos || {}).reduce((x, y) => x + y, 0), 0);
        ...
        <div>
          ...
          <div style={{ display:"grid", gap:5 }}>
            {viajesMes.length === 0
              ...
              : viajesMes.map(v => {
                const total = Object.values(v.gastos || {}).reduce((x, y) => x + y, 0);
                return (
                  <div key={v.id} style={{ fontSize:13, padding:"8px 12px", background:BG_VIAJE, borderRadius:9, border:`1px solid ${COLOR_VIAJE}33` }}>
                    ...
                    {Object.entries(v.gastos || {}).filter(([, val]) => val > 0).map(([k, val]) => (
