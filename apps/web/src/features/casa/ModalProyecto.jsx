...
import { HABITACIONES, PRIORIDADES, ESTADOS } from "../../constants/habitaciones.js";
import { C, inputS, labelS, cardN } from "../../constants/colores.js";
    ...
    <Modal onClose={onClose} maxW={480}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        <h3 style={{ fontSize:18, fontWeight:700, color:C.txt }}>
          {proyecto ? "Editar tarea" : "Nueva tarea"}
        </h3>
      ...
      <div style={{ display:"grid", gap:14 }}>
        {/* Habitación */}
        <div>
          <label style={{ ...labelS, color:C.txt2 }}>Habitación</label>
        ...
        {/* Título */}
        <div>
          <label style={{ ...labelS, color:C.txt2 }}>Título</label>
          <input value={form.titulo} onChange={e => set("titulo", e.target.value)} placeholder="Ej: Pintar paredes" style={{ ...inputS, background:C.fondo, border:`1px solid ${C.borde}` }}/>
        </div>

        {/* Descripción */}
        <div>
          <label style={{ ...labelS, color:C.txt2 }}>Descripción</label>
          <input value={form.descripcion || ""} onChange={e => set("descripcion", e.target.value)} placeholder="Detalles..." style={{ ...inputS, background:C.fondo, border:`1px solid ${C.borde}` }}/>
        </div>

        {/* Prioridad + Estado */}
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
          <div>
            <label style={{ ...labelS, color:C.txt2 }}>Prioridad</label>
          ...
          <div>
            <label style={{ ...labelS, color:C.txt2 }}>Estado</label>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          <div>
            <label style={{ ...labelS, color:C.txt2 }}>Inicio</label>
          ...
          <div>
            <label style={{ ...labelS, color:C.txt2 }}>Fin</label>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          <div>
            <label style={{ ...labelS, color:C.txt2 }}>Presupuesto (€)</label>
          ...
          <div>
            <label style={{ ...labelS, color:C.txt2 }}>Gastado (€)</label>
        ...
        {form.presupuesto > 0 && (
          <div style={{ background:C.fondo, borderRadius:10, padding:"10px 14px", border:`1px solid ${C.borde}` }}>
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:C.txt2, marginBottom:6 }}>
              <span>{fmt(+form.gasto || 0)} gastado</span>
        ...
        <div>
          ...
          <input value={form.notas || ""} onChange={e => set("notas", e.target.value)} placeholder="Observaciones..." style={{ ...inputS, background:C.fondo, border:`1px solid ${C.borde}` }}/>
        </div>
        ...
        <div style={{ display:"flex", gap:8 }}>
          ...
          {proyecto && (
            <button onClick={() => { if (window.confirm("¿Eliminar esta tarea?")) onDelete(proyecto.id); }}
