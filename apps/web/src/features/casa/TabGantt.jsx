...
import { C, card } from "../../constants/colores.js";
import { fmt } from "../../utils/format.js";
import { todayISO, toISO, daysBetween } from "../../utils/dates.js";
import { MESES_CORTO } from "../../constants/meses.js";
...
export default function TabGantt({ proyectos, setProyectos }) {
  ...
  const pctPos = (dateStr) => {
    ...
    return Math.max(0, Math.min(100, ((d - start) / (totalDias * 86400000)) * 100));
  };
  ...
  const porEstado   = useMemo(() => Object.keys(ESTADOS).reduce((acc, k) => ({ ...acc, [k]: proyectos.filter(p => p.estado === k).length }), {}), [proyectos]);
